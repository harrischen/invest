...
def error_response(code, msg):
    return json.dumps({"code": code, "market": "US", "status": "error",
...
def fetch_quote(code):
    """获取美股实时行情 - 腾讯 API"""
    try:
        r = HTTP_CLIENT.get(f"http://qt.gtimg.cn/q=us{code}")
        match = re.search(r'"(.+)"', r.text)
        if not match:
            return error_response(code, f"未找到美股 {code}")
        fields = match.group(1).split("~")
        if len(fields) < 30:
            return error_response(code, "数据格式异常")

        # 腾讯美股: [1]名称 [2]代码 [3]当前价 [4]昨收 [5]今开 [6]成交量
        # [33]最高 [34]最低 [39]PE [45]总市值(亿美元) [51]52周高 [52]52周低
        price = float(fields[3]) if fields[3] else 0
        prev_close = float(fields[4]) if fields[4] else 0
        change_pct = float(fields[32]) if len(fields) > 32 and fields[32] else round((price-prev_close)/prev_close*100, 2) if prev_close else 0

        return json.dumps({
            "code": code, "name": fields[1], "market": "US",
...
def fetch_technical(code):
    """获取美股技术面 - 腾讯K线"""
    try:
        # 美股K线需要 .OQ 后缀（纳斯达克）或 .N（纽交所）
        # 先尝试 .OQ，失败再试 .N
        kdata = []
        stock_key = None
        for suffix in [".OQ", ".N", ""]:
            sym = f"us{code}{suffix}"
            url = f"https://proxy.finance.qq.com/ifzqgtimg/appstock/app/newfqkline/get?param={sym},day,,,250,"
        ...
        lma5, lma10, lma20, lma60 = [float(m.iloc[-1]) if not pd.isna(m.iloc[-1]) else None for m in [ma5,ma10,ma20,ma60]]

        if all(v is not None for v in [lma5,lma10,lma20,lma60]):
            if lma5>lma10>lma20>lma60: ma_arr = "bullish"
            elif lma5<lma10<lma20<lma60: ma_arr = "bearish"
            else: ma_arr = "tangled"
        else: ma_arr = "unknown"
        ...
        rsv=(close-l9)/(h9-l9)*100; k=rsv.ewm(com=2,adjust=False).mean(); d=k.ewm(com=2,adjust=False).mean(); j=3*k-2*d

        # Boll
        bm=close.rolling(20).mean(); bs=close.rolling(20).std(); bu=bm+2*bs; bl=bm-2*bs
        ...
        return json.dumps({
            "code": code, "name": "", "market": "US", "timestamp": datetime.now().strftime("%Y-%m-%d"),
            "data": {
                "price": cp,
...
def fetch_finance(code):
    """美股财务数据（基础信息从行情获取，详细财务建议 WebSearch）"""
    try:
        quote_data = json.loads(fetch_quote(code))
        if quote_data["status"] != "ok":
            return error_response(code, "无法获取行情数据")
        return json.dumps({
            "code": code, "name": quote_data.get("name", ""), "market": "US",
            "timestamp": datetime.now().strftime("%Y-%m-%d"),
            "data": {
                "pe": quote_data["data"].get("pe"),
                "market_cap": quote_data["data"].get("total_market_cap"),
                "note": "美股详细财务数据（营收/利润/现金流）建议通过 WebSearch 获取最新财报"
            }, "status": "ok"
        }, ensure_ascii=False, indent=2)
    except Exception as e:
...
def fetch_valuation(code):
    """美股估值"""
    return fetch_quote(code)  # 腾讯行情已包含 PE/市值/52周高低
...
def fetch_all(code):
    ...
    return json.dumps({"code": code, "market": "US",
                       "timestamp": datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
...
def main():
    if len(sys.argv) < 3:
        print(json.dumps({"status": "error", "error": "用法: python3 fetch_us_stock.py <ticker> <类型>"}, ensure_ascii=False, indent=2))
        sys.exit(1)
    code = sys.argv[1].upper()
    data_type = sys.argv[2]
    dispatch = {"quote": fetch_quote, "finance": fetch_finance, "technical": fetch_technical,
                "valuation": fetch_valuation, "all": fetch_all}
